...
- **2026-08-18** (branch `mobile-changes`) — `.ds-h3-muted` ("Senior Product Designer" on the homepage) moved from `--type-h2` to `--type-h1` (20/600/1.4 → 24/600/1.33). Color unchanged (`var(--light-2)`). Despite the generic class name, `.ds-h3-muted` is only used on `index.html`, so this is homepage-only. This re-applies the same mapping the class had earlier in the session (see the 2026-08-18 entry that moved it `--type-h1` → `--type-h2`); a companion change moving `.bio-company` to `--type-caption` was trialled alongside it and reverted, so the location line stays on `--type-body`.
